...
public:
    ...
    //initial size is used to preallocate memory for the internal unordered map
    HashGrid(const Eigen::Vector3f &cellExtents, const int initialSize) : cellHashMap(initialSize),
    ...
    void InsertLine(const LineSegment &lineSegment) {
        Box bbox = lineSegment.ComputeBounds();
        Eigen::Vector3f v0 = lineSegment.getV0();
        Eigen::Vector3f v1 = lineSegment.getV1();
        Eigen::Vector3f currentPosition = v0;

        Eigen::Vector3f dir = v1 - v0;
        Eigen::Vector3i currentGrid = PositionToCellIndex(v0, cellExtents);
        Eigen::Vector3i endGrid = PositionToCellIndex(v1, cellExtents);

        cellHashMap[currentGrid].push_back(lineSegment);
        while ((currentGrid.x() != endGrid.x() || currentGrid.y() != endGrid.y() || currentGrid.z() != endGrid.z()) &&
               bbox.IsInside(currentPosition)) {
            CalculateNextGrid(currentGrid, dir, cellExtents, currentPosition);
            cellHashMap[currentGrid].push_back(lineSegment);
        }
