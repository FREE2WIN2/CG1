...
void CreateCube(HEMesh &mesh) {
    ...
    vhandles[0] = mesh.add_vertex(OpenMesh::Vec3f(0, 0, 1));
    vhandles[1] = mesh.add_vertex(OpenMesh::Vec3f(1, 0, 1));
    vhandles[2] = mesh.add_vertex(OpenMesh::Vec3f(1,1, 1));
    vhandles[3] = mesh.add_vertex(OpenMesh::Vec3f(0, 1, 1));

    vhandles[4] = mesh.add_vertex(OpenMesh::Vec3f(0, 0, 0));
    vhandles[5] = mesh.add_vertex(OpenMesh::Vec3f(1, 0, 0));
    vhandles[6] = mesh.add_vertex(OpenMesh::Vec3f(1,1, 0));
    vhandles[7] = mesh.add_vertex(OpenMesh::Vec3f(0, 1, 0));
...
void CreateIcosahedron(HEMesh &mesh, float radius) {
    ...
    for (int i = 0; i < 5; i++)
        vhandles[k++] = mesh.add_vertex(OpenMesh::Vec3f((float) cos(i * 72.0 * 3.14159 / 180.0) * r2, h,
                                                        -(float) sin(i * 72.0 * 3.14159 / 180.0) * r2));

    for (int i = 0; i < 5; i++)
        vhandles[k++] = mesh.add_vertex(
                OpenMesh::Vec3f((float) cos(36.0 * 3.14159 / 180.0 + i * 72.0 * 3.14159 / 180.0) * r2, -h,
...
void CreateOctahedron(HEMesh &mesh, float radius) {
    ...
    for (int i = 0; i < 4; i++)
        vhandles[k++] = mesh.add_vertex(
                OpenMesh::Vec3f((float) cos(i * 3.14159 / 2.0) * radius, 0, -(float) sin(i * 3.14159 / 2.0) * radius));
...
//create a unit arrow
void
CreateUnitArrow(HEMesh &mesh, float stem_radius, float head_radius, float stem_height, int slices, int stem_stacks) {
    ...
    for (int j = 0; j < slices; j++) {
        float angle2 = j * 2.0f * 3.14159f / (float) (slices);
        vhandles[k] = mesh.add_vertex(
                OpenMesh::Vec3f(cos(angle2) * head_radius, stem_height, sin(angle2) * head_radius));
        k++;
    }
